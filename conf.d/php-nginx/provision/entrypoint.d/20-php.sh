...
RUN_INSTALLER_FAQ=${RUN_INSTALLER_FAQ:-'false'}
...
if [[ -n $EZ_INSTANCE ]]; then
    if [[ -f vendor/bin/ocinstall ]]; then
        if [[ $RUN_INSTALLER == 'true' ]]; then
            ...
            php vendor/bin/ocinstall --allow-root-user -sbackend --embed-dfs-schema --no-interaction --languages=ita-IT,ita-PA ../installer/
        else
        ...
        if [[ $RUN_INSTALLER_TRASPARENZA == 'true' ]]; then
            ...
            php vendor/bin/ocinstall --allow-root-user -sbackend --no-interaction ../installer/modules/trasparenza
        else
            echo "[info] RUN_INSTALLER_TRASPARENZA is set to false"
        fi
        if [[ $RUN_INSTALLER_NEWSLETTER == 'true' ]]; then
            echo "[info] run installer trasparenza on ${EZ_INSTANCE}"
            php vendor/bin/ocinstall --allow-root-user -sbackend --no-interaction ../installer/modules/newsletter
        else
        ...
        if [[ $RUN_INSTALLER_DEMO == 'true' ]]; then
            ...
            php vendor/bin/ocinstall --allow-root-user -sbackend --no-interaction ../installer/modules/demo
        else
            echo "[info] RUN_INSTALLER_DEMO is set to false"
        fi
        if [[ $RUN_INSTALLER_FAQ == 'true' ]]; then
            echo "[info] run installer faq on ${EZ_INSTANCE}"
            php vendor/bin/ocinstall --allow-root-user -sbackend --no-interaction ../installer/modules/faq
        else
            echo "[info] RUN_INSTALLER_FAQ is set to false"
        fi
    else
